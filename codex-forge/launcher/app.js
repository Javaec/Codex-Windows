...
function renderMods(state) {
  document.getElementById("mod-counts").textContent = `${state.modCounts.enabled}/${state.modCounts.total} enabled`;
  ...
  container.innerHTML = state.mods.map((mod) => `
    <label class="mod-row">
      ...
      <div>
        ...
        <div class="runtime-chip">${mod.runtimeInstalled ? "synced to runtime" : "source only"}</div>
      </div>
    </label>
  `).join("");
