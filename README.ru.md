...
Launcher не изменяет Store-пакет или пользовательские данные. Перед запуском он проверяет версию и SHA-256 текущей сборки `26.715.2305.0`, затем временно подменяет только `@worklouder/device-kit-oai`, чтобы `findWLDevices()` возвращал пустой список. Work Louder / Codex Micro при этом не работает.
...
После обновления приложения launcher намеренно завершится с ошибкой до запуска: это временный workaround, и для новой сборки потребуется новый проверенный fingerprint.
