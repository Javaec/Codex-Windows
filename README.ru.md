...
Перед запуском launcher очищает унаследованные служебные `CODEX_*`-переменные сессии и временные inspector-аргументы, чтобы новый app-server стартовал как чистый Store-запуск и не привязался к текущему чату. Сохраняются только пользовательский `CODEX_HOME`, ключи провайдера и настроенные пути runtime; путь CLI выбирается самим приложением.
