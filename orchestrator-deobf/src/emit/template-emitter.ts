...
function buildGeneratedDesktopRepackHostRunner(): string {
  return [
    'import fs from "node:fs";',
    'import path from "node:path";',
    'import { spawn } from "node:child_process";',
    ...
    "",
    "const PROJECT_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), \"..\");",
    "const WORK_ROOT = path.resolve(PROJECT_ROOT, \"..\", \"..\", \"..\", \"..\", \"work\");",
    "const HOST_APP_ROOT = path.resolve(",
    "  process.env.CODEX_HOST_APP_ROOT && process.env.CODEX_HOST_APP_ROOT.trim().length > 0",
    "    ? process.env.CODEX_HOST_APP_ROOT",
    "    : path.join(WORK_ROOT, \"app\"),",
    ");",
    ...
    "  \"I am the router\",",
    "  \"connection state changed hostId=local next=connected\",",
    "];",
    "",
    "function requireExistingPath(targetPath, label) {",
    "  if (!fs.existsSync(targetPath)) {",
    "    throw new Error(`generated repack host: missing ${label}: ${targetPath}`);",
    "  }",
    "}",
    "",
    "function resolveElectronExePath() {",
    "  const candidates = [",
    "    process.env.ELECTRON_EXE_PATH ?? \"\",",
    "    path.join(WORK_ROOT, \"ci-10711\", \"native-builds\", \"node_modules\", \"electron\", \"dist\", \"electron.exe\"),",
    "    path.join(WORK_ROOT, \"ci-10711\", \"native-builds\", \"node_modules\", \"electron\", \"dist\", \"electron\"),",
    "    path.join(WORK_ROOT, \"native-builds\", \"node_modules\", \"electron\", \"dist\", \"electron.exe\"),",
    "    path.join(WORK_ROOT, \"native-builds\", \"node_modules\", \"electron\", \"dist\", \"electron\"),",
    "  ].map((value) => value.trim()).filter((value) => value.length > 0);",
    "  for (const candidate of candidates) {",
    "    if (fs.existsSync(candidate)) {",
    "      return path.resolve(candidate);",
    ...
    "  }",
    "  throw new Error(",
    "    \"generated repack host: electron executable not found. \" +",
    "    \"Set ELECTRON_EXE_PATH or run root runner once to build native electron.\",",
    "  );",
    "}",
    "",
    ...
    "    path.resolve(PROJECT_ROOT, \"..\", \"..\", \"..\", \"..\", \"dist\", \"Codex-win32-x64\", \"resources\", \"codex.exe\"),",
    "    path.resolve(PROJECT_ROOT, \"..\", \"..\", \"..\", \"..\", \"dist\", \"Codex-win32-arm64\", \"resources\", \"codex.exe\"),",
    "  ].map((value) => value.trim()).filter((value) => value.length > 0);",
    "  for (const candidate of candidates) {",
    "    if (fs.existsSync(candidate)) {",
    "      return path.resolve(candidate);",
    ...
    "}",
    "",
    "function lineIncludesReadyMarker(line) {",
    "  return READY_MARKERS.some((marker) => line.includes(marker));",
    "}",
    "",
    "async function run() {",
    "  requireExistingPath(HOST_APP_ROOT, \"host app root\");",
    "  requireExistingPath(path.join(HOST_APP_ROOT, \"package.json\"), \"host app package\");",
    "  requireExistingPath(path.join(HOST_APP_ROOT, \".vite\", \"build\", \"main.js\"), \"host main bundle\");",
    "  requireExistingPath(path.join(HOST_APP_ROOT, \"webview\", \"index.html\"), \"host webview index\");",
    "  const electronExe = resolveElectronExePath();",
    ...
    "  }",
    "  process.stdout.write(",
    "    `[generated-repack-host] mode=${IS_SMOKE ? \"smoke\" : \"run\"} electron=${electronExe} appRoot=${HOST_APP_ROOT}\\n`,",
    "  );",
    "  const child = spawn(electronExe, [HOST_APP_ROOT], {",
    ...
    "    const text = chunk.toString();",
    "    process.stdout.write(text);",
    "    if (!ready && lineIncludesReadyMarker(text)) {",
    "      ready = true;",
    "      if (IS_SMOKE) {",
    "        process.stdout.write(\"[generated-repack-host] readiness marker observed\\n\");",
    "        setTimeout(() => {",
    "          if (!child.killed) {",
    "            child.kill();",
    "          }",
    ...
    "          child.kill();",
    "        }",
    "        reject(new Error(`generated repack host: startup timeout after ${STARTUP_TIMEOUT_MS}ms`));",
    "      }, STARTUP_TIMEOUT_MS);",
    "    }",
    "    child.once(\"error\", (error) => {",
    "      if (timeout) {",
    ...
    "      if (IS_SMOKE && !ready) {",
    "        reject(new Error(`generated repack host: readiness marker not observed (exit=${resolvedCode})`));",
    "        return;",
    "      }",
    "      if (!IS_SMOKE && resolvedCode !== 0) {",
    "        reject(new Error(`generated repack host: process exited with code ${resolvedCode}`));",
    "        return;",
    "      }",
    "      resolve();",
    "    });",
    "  });",
    "  process.stdout.write(\"[generated-repack-host] ok\\n\");",
    "}",
    "",
    "run().catch((error) => {",
    "  const message = error instanceof Error ? error.stack ?? error.message : String(error);",
...
export async function emitTemplateProject(
  ...
  const desktopHostPath = path.join(outputProjectDirectory, "runtime", "desktop-repack-host.mjs");
  await writeTextFile(desktopHostPath, buildGeneratedDesktopRepackHostRunner());
  emittedFiles.push(toProjectRelative(outputProjectDirectory, desktopHostPath));
