...
const WEBVIEW_SETTINGS_LIMIT_PANEL_PATCH_TAG = "/* CODEX-WINDOWS-SETTINGS-LIMIT-PANEL-V2 */";
const WEBVIEW_SETTINGS_LIMIT_PANEL_SCRIPT = String.raw`(function(){if(typeof window==="undefined"||typeof document==="undefined")return;const globalRecord=window;if(globalRecord.__CODEX_WINDOWS_SETTINGS_LIMIT_PANEL_V2__)return;globalRecord.__CODEX_WINDOWS_SETTINGS_LIMIT_PANEL_V2__=true;const PANEL_ID="codex-windows-settings-limit-panel-v1";const STYLE_ID="codex-windows-settings-limit-panel-style-v1";const LIMITS_CACHE_KEY="codex-windows-limits-panel-cache-v1";const PANEL_HTML='<div class="codex-windows-limit-title">Usage limits</div><div class="codex-windows-limit-row"><span>5-hour limit</span><strong data-codex-limit-five-hour>-- / 5h</strong></div><div class="codex-windows-limit-row"><span>Weekly limit</span><strong data-codex-limit-weekly>-- / 7d</strong></div><div class="codex-windows-limit-updated" data-codex-limit-updated>waiting for data</div>';const panelState={fiveHour:null,weekly:null,updatedAtIso:""};const ensureStyle=()=>{if(document.getElementById(STYLE_ID))return;const style=document.createElement("style");style.id=STYLE_ID;style.textContent=".codex-windows-limit-panel{display:flex;flex-direction:column;gap:6px;margin:6px 0 10px;padding:10px 12px;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);font-size:12px;line-height:1.3}.codex-windows-limit-title{font-weight:600;opacity:.95}.codex-windows-limit-row{display:flex;align-items:center;justify-content:space-between;gap:10px}.codex-windows-limit-row strong{font-weight:700;letter-spacing:.2px}.codex-windows-limit-updated{opacity:.72;font-size:11px}";document.head?.appendChild(style)};const hasOwn=(target,key)=>Object.prototype.hasOwnProperty.call(target,key);const pickNumber=(target,keys)=>{for(const key of keys){if(!hasOwn(target,key))continue;const value=target[key];const numeric=typeof value==="number"?value:Number.parseFloat(String(value));if(Number.isFinite(numeric))return numeric}return null};const pickString=(target,keys)=>{for(const key of keys){if(!hasOwn(target,key))continue;const value=target[key];if(typeof value==="string"&&value.trim().length>0)return value.trim()}return""};const windowKindFromSeconds=value=>{if(typeof value!=="number"||!Number.isFinite(value))return"";if(value>=17000&&value<=19000)return"fiveHour";if(value>=580000&&value<=620000)return"weekly";return""};const windowKindFromLabel=value=>{const label=String(value||"").toLowerCase();if(!label)return"";if(label.includes("5h")||label.includes("five_hour")||label.includes("fivehour")||label.includes("short_window"))return"fiveHour";if(label.includes("week")||label.includes("7d")||label.includes("seven_day")||label.includes("weekly"))return"weekly";return""};const readUsageEntry=value=>{if(!value||typeof value!=="object"||Array.isArray(value))return null;const record=value;const used=pickNumber(record,["used","consumed","usage","count","requestsUsed","usedCount"]);const limit=pickNumber(record,["limit","max","quota","requestsLimit","limitCount"]);const remaining=pickNumber(record,["remaining","left","available","remainingCount"]);const resetAt=pickString(record,["resetAt","reset_at","resetTime","reset_time","windowResetAt"]);if(used===null&&limit===null&&remaining===null&&resetAt.length<1)return null;return{used,limit,remaining,resetAt}};const scoreEntry=entry=>{if(!entry)return 0;let score=0;if(entry.used!==null)score+=3;if(entry.limit!==null)score+=3;if(entry.remaining!==null)score+=2;if(entry.resetAt&&entry.resetAt.length>0)score+=1;return score};const chooseBetterEntry=(current,next)=>scoreEntry(next)>scoreEntry(current)?next:current;const parseSnapshotFromPayload=payload=>{const result={fiveHour:null,weekly:null};const stack=[{value:payload,label:""}];const seen=new Set();while(stack.length>0){const item=stack.pop();if(!item)continue;const current=item.value;if(!current||typeof current!=="object")continue;if(seen.has(current))continue;seen.add(current);if(Array.isArray(current)){for(let index=0;index<current.length;index+=1){stack.push({value:current[index],label:item.label})}continue}const record=current;const entry=readUsageEntry(record);const directLabel=pickString(record,["window","windowLabel","period","name","id","type"]);const kindByLabel=windowKindFromLabel(item.label)||windowKindFromLabel(directLabel);const kindBySeconds=windowKindFromSeconds(pickNumber(record,["windowSeconds","window_seconds","windowSec","window_sec","durationSeconds","duration_seconds","periodSeconds","period_seconds"]));const kind=kindBySeconds||kindByLabel;if(entry&&kind==="fiveHour")result.fiveHour=chooseBetterEntry(result.fiveHour,entry);if(entry&&kind==="weekly")result.weekly=chooseBetterEntry(result.weekly,entry);const keys=Object.keys(record);for(const key of keys){stack.push({value:record[key],label:key})}}return result};const formatEntry=(entry,fallback)=>{if(!entry)return fallback;if(entry.used!==null&&entry.limit!==null)return String(entry.used)+" / "+String(entry.limit);if(entry.remaining!==null&&entry.limit!==null)return String(Math.max(0,entry.limit-entry.remaining))+" / "+String(entry.limit);if(entry.remaining!==null)return String(entry.remaining)+" left";if(entry.limit!==null)return "0 / "+String(entry.limit);if(entry.resetAt&&entry.resetAt.length>0)return "resets "+String(entry.resetAt);return fallback};const renderPanel=()=>{const panel=document.getElementById(PANEL_ID);if(!(panel instanceof HTMLElement))return;const fiveHourNode=panel.querySelector("[data-codex-limit-five-hour]");const weeklyNode=panel.querySelector("[data-codex-limit-weekly]");const updatedNode=panel.querySelector("[data-codex-limit-updated]");if(fiveHourNode)fiveHourNode.textContent=formatEntry(panelState.fiveHour,"-- / 5h");if(weeklyNode)weeklyNode.textContent=formatEntry(panelState.weekly,"-- / 7d");if(updatedNode){if(panelState.updatedAtIso){updatedNode.textContent="updated "+String(panelState.updatedAtIso)}else{updatedNode.textContent="waiting for data"}}};const saveSnapshot=()=>{try{const payload={fiveHour:panelState.fiveHour,weekly:panelState.weekly,updatedAtIso:panelState.updatedAtIso};localStorage.setItem(LIMITS_CACHE_KEY,JSON.stringify(payload))}catch{}};const loadSnapshot=()=>{try{const raw=localStorage.getItem(LIMITS_CACHE_KEY);if(!raw)return;const parsed=JSON.parse(raw);if(parsed&&typeof parsed==="object"){panelState.fiveHour=readUsageEntry(parsed.fiveHour)||panelState.fiveHour;panelState.weekly=readUsageEntry(parsed.weekly)||panelState.weekly;if(typeof parsed.updatedAtIso==="string")panelState.updatedAtIso=parsed.updatedAtIso}}catch{}};const updateFromPayload=payload=>{const snapshot=parseSnapshotFromPayload(payload);let changed=false;if(snapshot.fiveHour){panelState.fiveHour=chooseBetterEntry(panelState.fiveHour,snapshot.fiveHour);changed=true}if(snapshot.weekly){panelState.weekly=chooseBetterEntry(panelState.weekly,snapshot.weekly);changed=true}if(!changed)return;panelState.updatedAtIso=new Date().toISOString();saveSnapshot();renderPanel()};const isSettingsLabel=value=>String(value??"").trim().toLowerCase()==="settings";const findSettingsAnchor=()=>{const candidates=document.querySelectorAll("button,[role='button'],a,div,span");for(const node of candidates){if(!(node instanceof HTMLElement))continue;if(!isSettingsLabel(node.textContent))continue;return node}return null};const resolveHost=node=>{const host=node.closest("button,[role='button'],a,li,div")??node;return host instanceof HTMLElement?host:null};const injectPanel=()=>{ensureStyle();const anchor=findSettingsAnchor();if(!anchor)return false;const host=resolveHost(anchor);if(!host||!host.parentElement)return false;const parent=host.parentElement;let panel=parent.querySelector("#"+PANEL_ID);if(!(panel instanceof HTMLElement)){panel=document.createElement("div");panel.id=PANEL_ID;panel.className="codex-windows-limit-panel";panel.innerHTML=PANEL_HTML;parent.insertBefore(panel,host)}renderPanel();return true};const parseTextPayload=text=>{const value=String(text||"").trim();if(value.length<2||value.length>500000)return null;if((value.startsWith("{")&&value.endsWith("}"))||(value.startsWith("[")&&value.endsWith("]"))){try{return JSON.parse(value)}catch{return null}}return null};const installFetchInterceptor=()=>{if(typeof globalRecord.fetch!=="function")return;const originalFetch=globalRecord.fetch.bind(globalRecord);globalRecord.fetch=async(...args)=>{const response=await originalFetch(...args);try{const clone=response.clone();const contentType=(clone.headers.get("content-type")||"").toLowerCase();if(contentType.includes("json")){const data=await clone.json();updateFromPayload(data)}else if(contentType.includes("text")){const text=await clone.text();const parsed=parseTextPayload(text);if(parsed)updateFromPayload(parsed)}}catch{}return response}};const installXhrInterceptor=()=>{if(typeof XMLHttpRequest==="undefined")return;const proto=XMLHttpRequest.prototype;if(!proto||typeof proto.send!=="function")return;const originalSend=proto.send;proto.send=function(...args){this.addEventListener("loadend",()=>{try{const contentType=(this.getResponseHeader("content-type")||"").toLowerCase();if(!contentType.includes("json")&&!contentType.includes("text"))return;const text=typeof this.responseText==="string"?this.responseText:"";const parsed=parseTextPayload(text);if(parsed)updateFromPayload(parsed)}catch{}});return originalSend.apply(this,args)}};loadSnapshot();injectPanel();installFetchInterceptor();installXhrInterceptor();const observer=new MutationObserver(()=>{injectPanel()});observer.observe(document.documentElement||document.body,{childList:true,subtree:true});window.setInterval(()=>{injectPanel();loadSnapshot();renderPanel()},2500)})();`;
