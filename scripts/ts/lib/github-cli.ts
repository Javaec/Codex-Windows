...
import { ensureDir, writeInfo } from "./exec";
...
export interface DownloadedGitHubCli {
  path: string;
  source: string;
  tag: string;
  cacheDir: string;
...
function getPreferredWindowsCliTriple(): "x86_64-pc-windows-msvc" | "aarch64-pc-windows-msvc" {
  return process.env.PROCESSOR_ARCHITECTURE === "ARM64" ? "aarch64-pc-windows-msvc" : "x86_64-pc-windows-msvc";
}

function getExpectedAssetNames(triple: string): string[] {
  return [
    `codex-${triple}.exe`,
...
function hasCachedAsset(targetPath: string, expectedSize: number): boolean {
...
export function isGitHubAlphaCliChannel(channel: string | undefined): boolean {
  const normalized = String(channel || "").trim().toLowerCase();
  return normalized === "alpha" || normalized === "github-alpha" || normalized === "latest-alpha";
}

export async function downloadLatestGitHubAlphaCodexCli(workDir: string): Promise<DownloadedGitHubCli> {
  ...
  const release = releases.find((entry) => {
    if (!entry || entry.draft || !entry.prerelease) return false;
    const tag = String(entry.tag_name || "");
    if (!/alpha/i.test(tag)) return false;
  ...
  const cacheDir = ensureDir(path.join(path.resolve(workDir), "downloads", "codex-cli", "github-alpha", releaseTag, triple));
  ...
  for (const assetName of expectedAssetNames) {
    ...
    if (hasCachedAsset(destinationPath, Number(asset.size ?? 0))) continue;
    writeInfo(`Downloading GitHub alpha CLI asset: ${asset.name}`);
