...
export interface ReferenceParityGapDomainRow {
  domain: string;
  label: string;
...
export interface ReferenceParityGapsReport {
...
export interface ReferenceParityHelpers {
  dedupeKeywords(values: Iterable<string>, max: number): string[];
  splitReferenceToken(value: string): string[];
}

function addObservedKeyword(pool: Set<string>, value: string, splitReferenceToken: (value: string) => string[]): void {
...
function getParityGapConfidenceTier(
  impactScore: number,
  thresholds: { critical: number; high: number },
...
export function buildReferenceParityGapsReport(input: {
