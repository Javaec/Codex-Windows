...
function appendUniqueExportName(target: string[], used: Set<string>, value: string): void {
  const sanitized = sanitizeExportIdentifierName(value);
  if (sanitized === "symbol_export") return;
  if (used.has(sanitized)) return;
  used.add(sanitized);
...
function buildSemanticRecoveryExportRows(input: {
  ...
  const emittedModuleBaseNameRaw = toRecoveryModuleBaseName(
  ...
  const sourceSymbolCandidates = input.symbols
    ...
    .map((item) => item.name);
  const combinedNames = Array.from(new Set([...structuredNames, ...sourceSymbolCandidates])).slice(
    0,
    Math.max(1, input.maxExports),
  );
