...
function countRowNameChanges(previousRows: RankedExportRow[], nextRows: RankedExportRow[]): number {
  if (previousRows.length === 0 || nextRows.length === 0) return 0;
  const previousBySourceSymbol = new Map<string, string>();
  ...
  for (const row of nextRows) {
    ...
    if (previousName !== row.name) changed += 1;
  }
  return changed;
}
...
export function buildWebStormTestProject(input: BuildWebStormTestProjectInput): WebStormTestProjectReport {
  ...
  const registerChunkTsModule = (bridgeInput: ChunkTsBridgeRow): void => {
    const sourceFile = normalizeDeobfSourceFile(bridgeInput.sourceFile);
    if (sourceFile.length === 0) {
    ...
    if (toPosixPath(bridgeInput.chunkArtifactPath) !== toPosixPath(ensured.chunkArtifactPath)) {
      throw new Error(
        `Chunk TS module artifact mismatch for ${sourceFile}: ${bridgeInput.chunkArtifactPath} vs ${ensured.chunkArtifactPath}`,
      );
    }

    const normalizedModulePath = toPosixPath(bridgeInput.chunkTsModulePath).replace(/^\.?\//, "");
    const row: ChunkTsBridgeRow = {
      sourceFile,
    ...
    if (current.chunkArtifactPath !== row.chunkArtifactPath || current.sourceFile !== row.sourceFile) {
      throw new Error(
        `Chunk TS module collision for ${row.chunkTsModulePath}: ${current.sourceFile}/${current.chunkArtifactPath} vs ${row.sourceFile}/${row.chunkArtifactPath}`,
      );
    }
  };
  ...
  for (const row of sortedTargets) {
    ...
    const importAliasShapedModule = shapeImportAliasesIterative({
      moduleBody: templateEmission.moduleBody,
      sourceFile: activeSourceFile,
      resolveAliasName: ({ importedSourceFile, importedSymbol }) =>
        resolveImportAliasHint(importedSourceFile, importedSymbol),
      iterations: 3,
    });
    ...
    if (exportQualityPass.passes > 0) {
      row.rationale.add(
        `export-quality-pass: passes=${exportQualityPass.passes}, targeted=${exportQualityPass.targetedRenamed}, hookTransport=${exportQualityPass.hookTransportRenamed}, service=${exportQualityPass.serviceRenamed}`,
      );
    }
    if (importAliasShapedModule.renamedAliases > 0) {
      row.rationale.add(
        `import-alias-shape: renamed=${importAliasShapedModule.renamedAliases}, passes=${importAliasShapedModule.passes}`,
      );
    }
  ...
  while (chunkTsModulePendingQueue.length > 0) {
    ...
    const rewrittenChunkImports = rewriteChunkImportsToTsBridge({
