import type { LiftDeclarationGraphNode, LiftedExportKind } from "./symbol-lifter";
...
function scorePathAlignment(name: string, emittedPath: string): number {
  const moduleTokens = new Set(splitIdentifierTokens(emittedPath));
  const nameTokens = splitIdentifierTokens(name);
  if (moduleTokens.size === 0 || nameTokens.length === 0) return 0;
  let hits = 0;
...
function scoreArchetypeNameFit(archetype: ModuleArchetype, row: ClusterCandidateRow, emittedPath: string): number {
  const name = row.name.toLowerCase();
  const alignment = scorePathAlignment(row.name, emittedPath);
  if (archetype === "hook") {
    if (/^use[A-Z]/.test(row.name) && row.kind === "function") return 3 + alignment;
    if (/(state|signal|registry|store|hook)/.test(name)) return 2 + alignment;
    return alignment - 1;
  }
  if (archetype === "ui") {
    if (/(component|ui|view|panel|modal|button|menu|classnames)/.test(name)) return 2 + alignment;
    if (row.kind === "class") return 1 + alignment;
    return alignment;
  }
  if (archetype === "transport") {
    if (/(transport|ipc|event|stream|connection|socket)/.test(name)) return 3 + alignment;
    return alignment - 1;
  }
  if (archetype === "store") {
    if (/(store|state|cache|registry|signal)/.test(name)) return 3 + alignment;
    return alignment;
  }
  if (/(service|manager|provider|client|runtime|factory)/.test(name)) return 2 + alignment;
  return alignment;
}

function isKindAllowed(contract: ModuleSynthesisContract, kind: LiftedExportKind): boolean {
  return contract.requiredSymbolKinds.includes(kind);
}

function pickBestGraphNode(nodes: LiftDeclarationGraphNode[]): LiftDeclarationGraphNode | undefined {
...
function addEdgeWeight(map: Map<string, number>, left: string, right: string, weight: number): void {
...
function buildDependencyEdges(input: {
  ...
  const bySymbol = new Map<string, ClusterCandidateRow>();
  for (const row of input.rows) bySymbol.set(row.sourceSymbol, row);
  ...
  for (const row of input.rows) {
    ...
    for (const reference of graph.references) {
      if (!bySymbol.has(reference)) continue;
  ...
  for (let index = 0; index < input.rows.length; index += 1) {
    ...
    for (let inner = index + 1; inner < input.rows.length; inner += 1) {
      ...
      if (overlap >= 2) addEdgeWeight(edgeWeights, left.sourceSymbol, right.sourceSymbol, 1);
...
function buildConnectedComponents(rows: ClusterCandidateRow[], edgeWeights: Map<string, number>): ClusterCandidateRow[][] {
  const adjacency = new Map<string, Set<string>>();
  for (const row of rows) adjacency.set(row.sourceSymbol, new Set<string>());
  ...
  const bySymbol = new Map<string, ClusterCandidateRow>();
  for (const row of rows) bySymbol.set(row.sourceSymbol, row);
  const visited = new Set<string>();
  const components: ClusterCandidateRow[][] = [];

  for (const row of rows) {
    if (visited.has(row.sourceSymbol)) continue;
    const queue = [row.sourceSymbol];
    visited.add(row.sourceSymbol);
    const component: ClusterCandidateRow[] = [];
    while (queue.length > 0) {
      const symbol = queue.shift();
      if (!symbol) continue;
      const current = bySymbol.get(symbol);
      if (current) component.push(current);
      const neighbors = adjacency.get(symbol) ?? new Set<string>();
...
function scoreRowForArchetype(row: ClusterCandidateRow, contract: ModuleSynthesisContract, emittedPath: string): number {
...
function scoreComponent(component: ClusterCandidateRow[], contract: ModuleSynthesisContract, emittedPath: string): number {
  let score = 0;
  const kindSet = new Set<LiftedExportKind>();
  ...
  for (const kind of contract.requiredSymbolKinds) {
    if (kindSet.has(kind)) score += 2;
  }
  score += component.length * 1.2;
  return score;
}

export function applyArchetypeAndCluster(input: {
  rows: ClusterCandidateRow[];
  declarationGraphByName: Map<string, LiftDeclarationGraphNode[]>;
  contract: ModuleSynthesisContract;
  emittedPath: string;
}): ClusterCandidateRow[] {
  if (input.rows.length <= 1) return input.rows;
  ...
  if (components.length === 0) return filtered;
