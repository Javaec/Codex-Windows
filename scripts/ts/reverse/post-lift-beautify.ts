...
import { sanitizeIdentifier } from "./identifier-utils";
...
function toCamelIdentifier(tokens: string[]): string {
...
function toPascalIdentifier(tokens: string[]): string {
  const cleaned = tokens
    .map((token) => token.replace(/[^A-Za-z0-9_$]/g, ""))
    .filter((token) => token.length > 0);
  if (cleaned.length === 0) return "";
...
function normalizeIdentifierToken(input: string): string {
  return input.replace(/[^A-Za-z0-9]/g, "").toLowerCase();
}

function isNoisyIdentifierToken(input: string): boolean {
  const token = normalizeIdentifierToken(input);
  if (["is", "has", "to", "on", "of", "by"].includes(token)) return false;
  if (token.length <= 1) return true;
  if (NOISY_IDENTIFIER_TOKENS.has(token)) return true;
  if (/^\d+$/.test(token)) return true;
  if (/^[a-z]{1,2}\d{0,2}$/.test(token)) return true;
  if (/^(?:fn|class|var)\d+$/.test(token)) return true;
  if (/^(?:tmp|temp|line|ref)\d+$/.test(token)) return true;
  if (/^x[0-9a-f]{2,}$/i.test(token)) return true;
  return false;
}
...
function scoreCallableNameQuality(input: { name: string; kind: "function" | "class" }): number {
  const name = input.name.trim();
  if (name.length === 0) return 0;
  let score = 1;
  if (!/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(name)) score -= 0.55;
  if (name.length < 3) score -= 0.75;
  else if (name.length < 5) score -= 0.3;
  if (/\$/.test(name)) score -= 0.45;
  ...
  if (/(?:runtime|handler|manager|service)\d+$/i.test(name)) score -= 0.45;
  if (input.kind === "class" && !/^[A-Z]/.test(name)) score -= 0.2;
  if (input.kind === "function" && /^use[A-Z]/.test(name)) score += 0.1;
  return Math.max(0, Math.min(1, score));
}

function shouldAutoRenameCallable(input: { name: string; kind: "function" | "class" }): boolean {
  const quality = scoreCallableNameQuality(input);
  if (quality < 0.9) return true;
  const normalized = normalizeIdentifierToken(input.name);
  if (/(?:fn|class|var)\d+$/.test(normalized)) return true;
  if (/(?:legacy|inline|internal)\d+$/.test(normalized)) return true;
  return false;
}

interface ModuleRenameBase {
  camel: string;
  pascal: string;
  tokens: string[];
  isHook: boolean;
...
function shouldRenameReferenceIdentifier(node: ts.Identifier): boolean {
  if (isDeclarationNameIdentifier(node)) return true;
  const parent = node.parent;
  if (!parent) return false;
...
function createModuleSemanticContext(moduleBody: string): {
  ...
  host.getSourceFile = (fileName, languageVersion) => {
    if (toPosixPath(fileName) !== moduleFile) return undefined;
...
function buildModuleRenameBase(emittedPath: string | undefined): ModuleRenameBase {
  ...
  const moduleTokens = toPosixPath(normalizedPath)
    ...
    .map((token) => normalizeIdentifierToken(token))
    .filter((token) => token.length > 0);
  const filtered = moduleTokens.filter(
    (token) =>
      token.length >= 3 &&
      ![
        "src",
  ...
  const baseCamel = sanitizeIdentifier(toCamelIdentifier(uniqueTokens.length > 0 ? uniqueTokens : ["module", "runtime"]));
...
interface CallableDeclarationRecord {
  name: string;
...
interface InitializerHints {
  hintName: string;
  hintTokens: string[];
}

function collectInitializerNameHints(node: ts.Node): InitializerHints {
  let hintName = "";
  const hints: string[] = [];
  const pushHint = (value: string): void => {
    const normalized = value.trim();
    if (!/^[A-Za-z_$][A-Za-z0-9_$]{2,}$/.test(normalized)) return;
    ...
    for (const token of extractIdentifierTokens(normalized)) {
      if (isNoisyIdentifierToken(token)) continue;
      hints.push(token);
    }
  };
...
function inferCallableKindFromInitializer(initializer: ts.Expression): "function" | "class" | undefined {
  const inferNode = (node: ts.Node): "function" | "class" | undefined => {
    if (ts.isFunctionExpression(node) || ts.isArrowFunction(node)) return "function";
    if (ts.isClassExpression(node)) return "class";
    if (ts.isParenthesizedExpression(node)) return inferNode(node.expression);
...
function collectCallableDeclarations(sourceFile: ts.SourceFile): CallableDeclarationRecord[] {
  const declarations: CallableDeclarationRecord[] = [];
  const pushDeclaration = (declaration: CallableDeclarationRecord): void => {
...
function collectStatementContextTokens(input: {
  ...
  const pushToken = (token: string): void => {
    if (isNoisyIdentifierToken(token)) return;
    tokens.push(token);
  };
  const visit = (node: ts.Node): void => {
...
function pickFunctionVerb(input: {
  module: ModuleRenameBase;
  sourceTokens: string[];
  contextTokens: string[];
}): string {
  if (input.module.isHook) return "use";
  const merged = [...input.sourceTokens, ...input.contextTokens];
  const hasToken = (token: string): boolean => merged.includes(token);

  if (hasToken("is")) return "is";
  if (hasToken("has")) return "has";
  if (hasToken("to")) return "to";
  if (hasToken("get") || hasToken("read") || hasToken("query")) return "get";
  if (hasToken("set") || hasToken("assign")) return "set";
  if (hasToken("create") || hasToken("make") || hasToken("init")) return "create";
  if (hasToken("build") || hasToken("compose")) return "build";
  if (hasToken("load") || hasToken("fetch") || hasToken("request")) return "load";
  if (hasToken("parse") || hasToken("decode")) return "parse";
  if (hasToken("encode") || hasToken("serialize")) return "serialize";
  if (hasToken("compute") || hasToken("derive") || hasToken("calculate")) return "compute";
  if (hasToken("format") || hasToken("normalize")) return "format";
  if (hasToken("register") || hasToken("bind")) return "register";
  if (hasToken("dispatch") || hasToken("emit") || hasToken("send") || hasToken("publish")) return "dispatch";
  if (hasToken("subscribe") || hasToken("listen") || hasToken("watch")) return "subscribe";
  if (hasToken("connect") || hasToken("open")) return "connect";
  if (hasToken("update") || hasToken("refresh")) return "update";
  if (input.module.isTransport) return "dispatch";
  if (input.module.isService) return "resolve";
  if (input.module.isStore) return "update";
  return "resolve";
}
...
function pickClassSuffix(input: {
  ...
  const has = (value: string): boolean => merged.includes(value);
  if (has("store") || has("state") || has("cache") || input.module.isStore) return "Store";
  if (has("adapter") || has("bridge") || has("tauri")) return "Adapter";
  if (has("controller") || has("route") || has("navigation")) return "Controller";
  if (has("client") || has("transport") || has("ipc") || has("rpc") || has("socket") || input.module.isTransport) return "Client";
  if (has("service") || has("api") || input.module.isService) return "Service";
  return "Runtime";
}
...
function buildCallableRenameBase(input: {
  declaration: CallableDeclarationRecord;
  module: ModuleRenameBase;
  sourceFile: ts.SourceFile;
}): string {
  const sanitizeHintName = (): string => {
    const rawHint = input.declaration.hintName.trim();
    if (rawHint.length === 0) return "";
    if (input.declaration.kind === "function") {
      ...
      return sanitizeIdentifier(hintName);
    }
    const tokens = extractIdentifierTokens(rawHint).filter((token) => !isNoisyIdentifierToken(token));
    if (tokens.length === 0) return "";
  ...
  const sourceTokens = dedupeTokens([...input.declaration.hintTokens, ...declarationTokens]).filter(
    (token) => !isNoisyIdentifierToken(token),
  );
  ...
  if (input.declaration.kind === "function") {
    ...
    if (verb === "use") return `use${subjectPascal}`;
  ...
  if (classBase.endsWith(suffix)) return classBase;
...
function renameWeakTopLevelDeclarations(input: {
  ...
  const occupiedNames = new Set(Array.from(declarationCounts.keys()).map((name) => sanitizeIdentifier(name)).filter((name) => name.length > 0));
  const symbolRenameMap = new Map<ts.Symbol, string>();
  ...
  const reserveName = (baseName: string, previousName: string): string => {
    const sanitizedBase = sanitizeIdentifier(baseName);
    ...
    while (occupiedNames.has(nextName) && nextName !== previousName && suffix < 2000) {
      nextName = sanitizeIdentifier(`${sanitizedBase}${suffix}`);
      suffix += 1;
    }
    if (nextName.length === 0 || nextName === previousName || occupiedNames.has(nextName)) return "";
  ...
  for (const declaration of callableDeclarations) {
    ...
    if (!symbol) continue;
    if (symbolRenameMap.has(symbol)) continue;
    if (declaration.isTopLevel && exported.has(declaration.name)) continue;
    if (!shouldAutoRenameCallable({ name: declaration.name, kind: declaration.kind })) continue;
  ...
  if (symbolRenameMap.size === 0) return input.moduleBody;
