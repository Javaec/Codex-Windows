...
export interface DeobfuscationTableReport {
  ...
  coverage: {
    ...
    obfuscatedVariableCandidates: number;
    mappedFiles: number;
    mappedSymbols: number;
    mappedVariables: number;
    variableCoveragePercent: number;
  };
...
interface ObfuscatedVariableCandidate {
  name: string;
  sourceFile: string;
  line: number;
  tokens: string[];
...
const MATCH_V2_RUNTIME = resolveMatchV2RuntimeConfig(process.env.REVERSE_MATCH_V2_VARIANT);
...
function pickFallbackRenameToken(input: {
  ...
  for (let index = referenceSegments.length - 1; index >= 0; index -= 1) {
    for (const token of extractNameTokens(referenceSegments[index] ?? "")) {
      if (token.length < 3 || isGenericRenameToken(token)) continue;
  ...
  const sourceStem = path.posix.basename(
    toPosixPath(input.sourceFile),
    path.posix.extname(toPosixPath(input.sourceFile)),
  );
  ...
  if (domainToken.length >= 3 && !isGenericRenameToken(domainToken)) return domainToken;
...
function isBroadCandidateFunctionName(name: string): boolean {
  if (name.length < 2) return false;
  if (name.length > 80) return false;
  if (/^\d+$/.test(name)) return false;
  if (/^(constructor|prototype|default|module|exports|render|then|catch|finally)$/i.test(name)) return false;
  if (/^__[A-Za-z0-9_]+__$/.test(name)) return false;
  return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(name);
}

function isBroadCandidateClassName(name: string): boolean {
  if (name.length < 2) return false;
  if (name.length > 80) return false;
  if (/^\d+$/.test(name)) return false;
  if (/^(default|module|exports)$/i.test(name)) return false;
  return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(name);
}

function isLikelyObfuscatedVariableName(name: string): boolean {
  if (name.length < 1) return false;
  if (name.length <= 2) return true;
  if (/^[$_]?[a-z][0-9]{1,3}$/.test(name)) return true;
  if (/^[$_]?[a-z]{1,3}$/.test(name) && !/^(ctx|key|id|url|api|env|tmp)$/.test(name.toLowerCase())) return true;
  if (/^[a-z][a-z0-9]{0,3}$/.test(name) && !/[aeiou]/i.test(name)) return true;
  return false;
}

function isBroadCandidateVariableName(name: string): boolean {
  if (name.length < 1) return false;
  if (name.length > 80) return false;
  if (/^\d+$/.test(name)) return false;
  if (/^(arguments|undefined|window|document|globalThis|module|exports|require|console|process)$/i.test(name)) return false;
  if (/^__[A-Za-z0-9_]+__$/.test(name)) return false;
  return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(name);
}
...
function collectObfuscatedSymbolsFromSource(input: {
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isClassDeclaration(node) && node.name) {
      ...
      if (ts.isArrowFunction(node.initializer) || ts.isFunctionExpression(node.initializer)) {
        ...
        if (propertyName.length > 0) pushCandidate("function", propertyName, node.name);
      }
      ...
      if (node.operatorToken.kind === ts.SyntaxKind.EqualsToken) {
        if (ts.isArrowFunction(node.right) || ts.isFunctionExpression(node.right)) {
          ...
          if (propertyName.length > 0) pushCandidate("function", propertyName, node.left.name);
        }
      }
    }
  ...
  if (mode === "broad") {
    const pushRegexCandidates = (regex: RegExp, kind: "class" | "function"): void => {
      regex.lastIndex = 0;
...
function collectObfuscatedVariablesFromSource(input: {
  ...
  source: string;
  mode?: "strict" | "broad";
}): ObfuscatedVariableCandidate[] {
  const out: ObfuscatedVariableCandidate[] = [];
  const mode = input.mode ?? "strict";
  const seen = new Set<string>();
  ...
  if (mode === "broad") {
    const pushRegexCandidates = (regex: RegExp): void => {
      regex.lastIndex = 0;
      let match: RegExpExecArray | null;
      while ((match = regex.exec(input.source)) !== null) {
        const name = match[1] ?? "";
        if (!isBroadCandidateVariableName(name)) continue;
        const position = sourceFile.getLineAndCharacterOfPosition(match.index);
        const line = position.line + 1;
        const key = `${name}|${line}`;
        if (seen.has(key)) continue;
...
function scoreSymbolOwnershipAlignment(input: {
  sourceLayer: ReturnType<typeof classifyRuntimeLayer>;
  referenceFile: string;
...
function createEmptyFileSignalProfile(): FileSignalProfile {
  return {
    contextKeywords: new Set<string>(),
    ast: 0,
    ipcRpc: 0,
...
function getEntrySourceFile(value: string): string {
...
function buildAggressiveSymbolName(input: {
  ...
  if (input.candidate.kind === "class") {
    ...
    if (fallback.length >= 3 && !isGenericRenameToken(fallback.toLowerCase())) return fallback;
  } else {
  ...
  if (input.candidate.kind === "class") {
    ...
    if (sourceFallback.length >= 3) return sourceFallback;
  } else {
  ...
  if (input.candidate.kind === "class") {
    ...
    if (layerFallback.length >= 3) return layerFallback;
    return "DomainClass";
  }
...
function extractRefinementTokens(entry: DeobfuscationTableEntry): string[] {
  ...
  for (let index = referenceParts.length - 1; index >= 0; index -= 1) {
    ...
    for (const token of extractNameTokens(part)) {
      ...
      if (isGenericRenameToken(token)) continue;
      tokens.push(token);
    }
  ...
  if (layer !== "unknown") tokens.push(layer);
...
function buildRefinedSymbolName(input: {
  ...
  const baseIdentifier =
    input.kind === "class" ? toPascalCaseIdentifier(input.baseName) : toCamelCaseIdentifier(input.baseName);
  ...
  if (input.kind === "class") return `${seed}V${ordinal}`;
  return `${seed}V${ordinal}`;
}

function refineSymbolNames(entries: DeobfuscationTableEntry[]): void {
  const symbolEntries = entries.filter((entry) => entry.kind !== "file");
  if (symbolEntries.length === 0) return;

  const usedNames = new Set<string>(symbolEntries.map((entry) => entry.deobfuscated));
  const byName = new Map<string, DeobfuscationTableEntry[]>();
  ...
  for (const bucket of byName.values()) {
    ...
    if (!requiresRefine) continue;

    bucket.sort((a, b) => {
      if (a.confidence !== b.confidence) return b.confidence - a.confidence;
      if (a.reference.score !== b.reference.score) return b.reference.score - a.reference.score;
...
function scoreSymbolEntryQuality(entry: DeobfuscationTableEntry): number {
  let score = entry.confidence * 100 + entry.reference.score;
  const rationaleText = entry.rationale.join(" | ").toLowerCase();
  if (rationaleText.includes("fallback: mass-fill-non-generic")) score -= 32;
  if (rationaleText.includes("fallback: high-recall-non-generic-fill")) score -= 16;
  if (rationaleText.includes("fallback: source-anchor-symbol-expansion")) score -= 8;
  if (rationaleText.includes("fallback: ownership-symbol-recovery-non-generic")) score -= 4;
  if (rationaleText.includes("fallback: primary-best")) score += 6;
  if (rationaleText.includes("source-anchor:") && !rationaleText.includes("source-anchor: none")) score += 2;
  return score;
}

function collapseBestSymbolEntries(entries: DeobfuscationTableEntry[]): DeobfuscationTableEntry[] {
...
function countMappedSymbolEntries(entries: DeobfuscationTableEntry[]): number {
  const seen = new Set<string>();
  for (const entry of entries) {
...
function isLowQualitySymbolEntry(entry: DeobfuscationTableEntry): boolean {
  if (entry.kind !== "class" && entry.kind !== "function") return false;
  const rationaleText = entry.rationale.join(" | ").toLowerCase();
  if (rationaleText.includes("fallback: aggressive-symbol-coverage")) return true;
...
function applySymbolQualityPass(input: {
  entries: DeobfuscationTableEntry[];
  fileSignals: Map<string, FileSignalProfile>;
  sourceReferenceAnchors: Map<string, SourceReferenceAnchor>;
  strictSymbolCandidatesByFile: Map<string, ObfuscatedSymbolCandidate[]>;
  symbolsByKind: {
    class: ReferenceSymbolRow[];
    function: ReferenceSymbolRow[];
  };
  referenceFileProfiles: ReferenceFileProfile[];
}): { reviewed: number; improved: number } {
  ...
  const candidateBuckets = new Map<string, ObfuscatedSymbolCandidate[]>();
  ...
  const usedNamesByKind = new Set<string>();
  ...
  let improved = 0;
  for (const entry of input.entries) {
    if (!isLowQualitySymbolEntry(entry)) continue;
    reviewed += 1;
    const symbolKind: "class" | "function" = entry.kind === "class" ? "class" : "function";

    const sourceFile = getEntrySourceFile(entry.sourceFile);
    const candidateKey = `${symbolKind}|${sourceFile}|${entry.obfuscated}`;
    const candidateBucket = candidateBuckets.get(candidateKey) ?? [];
    if (candidateBucket.length === 0) continue;
    ...
    const isAggressiveFallback = entry.rationale.some(
      (item) =>
        item.includes("fallback: aggressive-symbol-coverage") || item.includes("fallback: final-symbol-completion"),
    );
    const minimumAcceptableScore = isAggressiveFallback ? 1.6 : 2.2;
    if (best.score < minimumAcceptableScore) continue;
...
function toOwnershipLayer(sourceLayer: ReturnType<typeof classifyRuntimeLayer>): "main" | "renderer" | "services" | "tauri" | "unknown" {
  if (sourceLayer === "main" || sourceLayer === "main-worker" || sourceLayer === "preload") return "main";
  if (sourceLayer === "renderer" || sourceLayer === "renderer-worker") return "renderer";
  return "unknown";
}

function isLayerOwnershipAllowed(input: {
  sourceLayer: ReturnType<typeof classifyRuntimeLayer>;
  referenceLayer: ReturnType<typeof inferReferenceLayer>;
  fileSignals: FileSignalProfile;
  anchorBoost: number;
}): boolean {
  const sourceOwnershipLayer = toOwnershipLayer(input.sourceLayer);
  if (sourceOwnershipLayer === "unknown") return input.anchorBoost >= 1.9;
  if (input.referenceLayer === "unknown") return input.anchorBoost >= 1.1;

  if (sourceOwnershipLayer === "renderer") {
    if (input.referenceLayer === "renderer") return true;
    if (input.referenceLayer === "services") return input.fileSignals.state >= 3 || input.fileSignals.ipcRpc >= 2;
  ...
  if (sourceOwnershipLayer === "main") {
    if (input.referenceLayer === "main" || input.referenceLayer === "tauri") return true;
    if (input.referenceLayer === "services") return input.fileSignals.state >= 2 || input.fileSignals.ipcRpc >= 2;
    if (input.referenceLayer === "renderer") {
      if (input.fileSignals.uiLikelihood >= 0.9 && input.fileSignals.flow >= 25) return true;
...
function getLayerPairKey(sourceLayer: ReturnType<typeof classifyRuntimeLayer>, referenceLayer: ReturnType<typeof inferReferenceLayer>): string {
  return `${toOwnershipLayer(sourceLayer)}|${referenceLayer}`;
}

function getLayerPairLimit(sourceLayer: ReturnType<typeof classifyRuntimeLayer>, referenceLayer: ReturnType<typeof inferReferenceLayer>): number {
  const sourceOwnershipLayer = toOwnershipLayer(sourceLayer);
  if (sourceOwnershipLayer === "renderer" && referenceLayer === "renderer") return 20;
  if (sourceOwnershipLayer === "renderer" && referenceLayer === "services") return 16;
  if (sourceOwnershipLayer === "renderer" && (referenceLayer === "main" || referenceLayer === "tauri")) return 12;
  if (sourceOwnershipLayer === "main" && (referenceLayer === "main" || referenceLayer === "tauri")) return 20;
  if (sourceOwnershipLayer === "main" && referenceLayer === "services") return 14;
  if (sourceOwnershipLayer === "main" && referenceLayer === "renderer") return 8;
  return 10;
}

function getHighRecallLayerPairLimit(
  sourceLayer: ReturnType<typeof classifyRuntimeLayer>,
  referenceLayer: ReturnType<typeof inferReferenceLayer>,
): number {
...
function isFileCandidateForPlan(input: {
  ...
  const obviousObfuscated =
    isLikelyObfuscatedClassName(bareName) || /-(?:[A-Za-z0-9]{6,})\.(?:js|mjs|cjs)$/i.test(baseName);
  if (obviousObfuscated) return true;

  const bundlerChunkLike =
    /^(?:index|main|chunk|worker|desktop|channel|clone|data-controls|diff|agent-settings|automation|git-settings|init)-[A-Za-z0-9]{6,}\.(?:js|mjs|cjs)$/i.test(
      baseName,
    );
...
export function buildDeobfuscationTableMatchV2(input: BuildDeobfuscationTableMatchV2Input): DeobfuscationTableReport {
  ...
  for (const symbol of referenceSymbolProfile.symbols) {
    if (symbol.symbolKind !== "class" && symbol.symbolKind !== "function") continue;
  ...
  const strictSymbolCandidatesByFile = new Map<string, ObfuscatedSymbolCandidate[]>();
  const uniqueObfuscatedSymbolCandidateKeys = new Set<string>();
  ...
  const symbolOwnershipPairCounts = new Map<string, number>();
  const symbolTargetByFile = new Set<string>();
  const getMappedSymbolCount = (): number => countMappedSymbolEntries(entries);
  ...
  for (const file of input.jsFiles) {
    ...
    if (isFileCandidateForPlan({ relPath, signal, sourceAnchor })) {
      ...
      if (bestProfile && isGenericReferenceFilePath(bestProfile.file)) {
        const fallbackRows: Array<{ profile: ReferenceFileProfile; score: number; hits: string[] }> = [];
        for (const profile of referenceFileProfiles) {
        ...
        const fallbackAnchored =
          ...
          fallbackRows.find(
            (row) =>
              row.profile.file === sourceAnchor.primaryFile || sourceAnchor.secondaryFiles.includes(row.profile.file),
          );
        const fallbackRow =
          fallbackAnchored && fallbackAnchored.score >= ((fallbackPrimary?.score ?? 0) - 1.3) ? fallbackAnchored : fallbackPrimary;
      ...
      if (!mappedFilePlan && sourceAnchor && signalStrength >= 9) {
        const anchorCandidates = [sourceAnchor.primaryFile, ...sourceAnchor.secondaryFiles].filter(
          (candidate) => !isGenericReferenceFilePath(candidate),
        );
        let fallbackAnchor: { profile: ReferenceFileProfile; score: number; hits: string[] } | undefined;
      ...
      if (!mappedFilePlan && signalStrength >= 8) {
        let floorFallback: { profile: ReferenceFileProfile; score: number; hits: string[] } | undefined;
        for (const profile of referenceFileProfiles) {
          if (isGenericReferenceFilePath(profile.file)) continue;
          const scored = scoreReferenceFileProfile({ sourceFile: relPath, profile, fileSignals: signal });
    ...
    for (const candidate of symbolCandidates) {
      ...
      if (targetFileMatchCount >= 12 && !hasAnchor) continue;
      ...
      if (ownershipPairCount >= getLayerPairLimit(sourceLayer, referenceLayer)) continue;
  ...
  if (getMappedSymbolCount() < targetMappedSymbols) {
    ...
    for (const entry of entries) {
      ...
      if (entrySourceFile.length > 0) mappedSourceFiles.add(entrySourceFile);
    }
    const symbolRecoveryRows: Array<{
      candidate: ObfuscatedSymbolCandidate;
      reference: ReferenceSymbolRow;
      score: number;
      hits: string[];
    ...
    for (const file of input.jsFiles) {
      ...
      for (const candidate of symbolCandidates) {
        ...
        for (const reference of referencePool) {
          if (isGenericReferenceFilePath(reference.file)) continue;
          const existingTargetKey = `${candidate.sourceFile}|${candidate.kind}|${reference.name}`;
          if (symbolTargetByFile.has(existingTargetKey)) continue;
          const scored = scoreReferenceSymbolMatch({
            sourceFile: relPath,
            candidate,
            reference,
        ...
        if (best.hits.length < 1 && getTotalSignalStrength(signal) < 8) continue;
        if (!isMappedSourceFile && best.hits.length < 1) continue;
    ...
    for (const row of symbolRecoveryRows) {
      if (getMappedSymbolCount() >= targetMappedSymbols) break;
      ...
      const matchedCount = symbolMatchCountByReference.get(referenceKey) ?? 0;
      if (matchedCount >= 8) continue;

      const sourceTargetKey = `${candidate.sourceFile}|${reference.file}`;
      const sourceTargetCount = symbolMatchCountBySourceTarget.get(sourceTargetKey) ?? 0;
      if (sourceTargetCount >= 4) continue;
      ...
      if (targetFileMatchCount >= 14) continue;

      const perFileCount = symbolMatchCountByFile.get(candidate.sourceFile) ?? 0;
      if (perFileCount >= 48) continue;
      if (!row.isMappedSourceFile && perFileCount >= 2) continue;
  ...
  if (getMappedSymbolCount() < targetMappedSymbols) {
    ...
    for (const sourceFile of mappedSourceFiles) {
      ...
      if (!sourceAnchor || sourceAnchor.score < 7.5) continue;
      ...
      if (candidateReferenceFiles.length === 0) continue;

      for (const candidate of symbolCandidates) {
        const mappedKey = `${candidate.kind}|${candidate.sourceFile}:${candidate.line}|${candidate.name}`;
        if (mappedCandidateKeys.has(mappedKey)) continue;
        ...
        for (const candidateReferenceFile of candidateReferenceFiles) {
          ...
          for (const reference of referencePool) {
            ...
            const sourceTargetKey = `${candidate.sourceFile}|${reference.file}`;
            const sourceTargetCount = symbolMatchCountBySourceTarget.get(sourceTargetKey) ?? 0;
            if (sourceTargetCount >= 5) continue;
            const scored = scoreReferenceSymbolMatch({
              sourceFile,
              candidate,
              reference,
        ...
        if (best.score < minScore) continue;
        if (best.hits.length < 1 && getTotalSignalStrength(signal) < 12) continue;
    ...
    expansionRows.sort((a, b) => {
      if (a.score !== b.score) return b.score - a.score;
      if (a.sourceAnchor.score !== b.sourceAnchor.score) return b.sourceAnchor.score - a.sourceAnchor.score;
      if (a.candidate.sourceFile !== b.candidate.sourceFile) {
        return a.candidate.sourceFile.localeCompare(b.candidate.sourceFile);
      }
      return a.candidate.name.localeCompare(b.candidate.name);
    ...
    const expansionCountBySource = new Map<string, number>();
  ...
  if (getMappedSymbolCount() < targetMappedSymbols) {
    ...
    const highRecallCountBySource = new Map<string, number>();
    for (const row of highRecallRows) {
      if (getMappedSymbolCount() >= targetMappedSymbols) break;

      const sourceFile = row.candidate.sourceFile;
      const sourceRecallCount = highRecallCountBySource.get(sourceFile) ?? 0;
      if (sourceRecallCount >= 24) continue;

      const perFileCount = symbolMatchCountByFile.get(sourceFile) ?? 0;
      if (perFileCount >= 120) continue;

      const referenceKey = `${row.reference.source}|${row.reference.name}|${row.reference.file}`;
      const matchedCount = symbolMatchCountByReference.get(referenceKey) ?? 0;
      if (matchedCount >= 24) continue;

      const sourceTargetKey = `${sourceFile}|${row.reference.file}`;
      const sourceTargetCount = symbolMatchCountBySourceTarget.get(sourceTargetKey) ?? 0;
      if (sourceTargetCount >= 12) continue;

      const targetFileMatchCount = symbolMatchCountByTargetFile.get(row.reference.file) ?? 0;
      if (targetFileMatchCount >= 40) continue;
  ...
  if (getMappedSymbolCount() < targetMappedSymbols) {
    ...
    const poolOffsets = new Map<string, number>();
    const nextFromPool = (poolKey: string, pool: ReferenceSymbolRow[]): ReferenceSymbolRow | undefined => {
    ...
    const getLayerOrder = (sourceLayer: ReturnType<typeof classifyRuntimeLayer>): Array<"main" | "renderer" | "services" | "tauri"> => {
    ...
    for (const file of input.jsFiles) {
      ...
      for (const candidate of candidates) {
        ...
        if (mappedCandidateKeys.has(candidateKey)) continue;

        const layerOrder = getLayerOrder(sourceLayer);
        let selectedReference: ReferenceSymbolRow | undefined;
        for (const layer of layerOrder) {
          selectedReference = nextFromPool(
            `mass-fill|${candidate.kind}|${layer}`,
            referencePools[candidate.kind][layer],
          );
          if (selectedReference) break;
        }
        ...
        const hits = dedupeKeywords(
          [...candidate.tokens, ...extractNameTokens(selectedReference.name), signal.dominantDomain],
          6,
        );
        const targetProjectPath = buildSignalAwareTargetPath({
  ...
  if (getMappedSymbolCount() < targetMappedSymbols) {
    ...
    const nonGenericProfilesByFile = new Map<string, ReferenceFileProfile>();
    const topProfileByLayer: Record<"main" | "renderer" | "services" | "tauri" | "unknown", ReferenceFileProfile | undefined> = {
      main: undefined,
      renderer: undefined,
      services: undefined,
      tauri: undefined,
    ...
    const getAggressiveLayerOrder = (
      sourceLayer: ReturnType<typeof classifyRuntimeLayer>,
    ): Array<"main" | "renderer" | "services" | "tauri"> => {
      if (sourceLayer === "renderer" || sourceLayer === "renderer-worker") return ["renderer", "services", "main", "tauri"];
      if (sourceLayer === "main" || sourceLayer === "main-worker" || sourceLayer === "preload") return ["main", "tauri", "services", "renderer"];
      return ["services", "renderer", "main", "tauri"];
    };
    ...
    const selectAggressiveProfile = (input: {
      ...
      signal: FileSignalProfile;
      sourceAnchor?: SourceReferenceAnchor;
    }): ReferenceFileProfile | undefined => {
      if (nonGenericProfiles.length === 0) return undefined;

      let best: { profile: ReferenceFileProfile; score: number } | undefined;
      ...
      if (best && best.score >= 0.35) return best.profile;
      ...
      for (const layer of getAggressiveLayerOrder(sourceLayer)) {
        ...
        if (layerProfile) return layerProfile;
      }
      return topProfileByLayer.unknown;
    };

    const aggressiveCountBySource = new Map<string, number>();
    for (const file of input.jsFiles) {
      if (getMappedSymbolCount() >= targetMappedSymbols) break;
      const relPath = file.relPath;
      if (!isDeobfuscationCandidateFile(relPath)) continue;
      ...
      for (const candidate of strictCandidates) {
        if (getMappedSymbolCount() >= targetMappedSymbols) break;
        if (sourceAggressiveCount >= 4000) break;
        ...
        const confidence = Math.min(
          0.68,
          roundMetric(0.12 + Math.min(14, selectedProfile.maxScore) / 28 + getTotalSignalStrength(signal) / 240),
        );
        const entrySource = `${candidate.sourceFile}:${candidate.line}`;
  ...
  if (getMappedSymbolCount() < uniqueObfuscatedSymbolCandidateKeys.size) {
    ...
    for (const [sourceFile, candidates] of strictSymbolCandidatesByFile) {
      if (getMappedSymbolCount() >= uniqueObfuscatedSymbolCandidateKeys.size) break;
      if (candidates.length === 0) continue;
...

  {
    ...
    for (const file of input.jsFiles) {
      ...
      const sourceAnchor = sourceReferenceAnchors.get(relPath);
      const signalStrength = getTotalSignalStrength(signal);
      if (signalStrength < 3 && !sourceAnchor) continue;
    ...
    const targetMappedVariables = Math.min(
      ...
      Math.max(80, Math.floor(obfuscatedVariableCandidates * 0.62)),
    );
  ...
  const variableCoveragePercent =
    obfuscatedVariableCandidates > 0
