...
function loadReferenceSymbolProfile(reportDir) {
...
function isObfuscatedFilePath(filePath) {
    const normalized = toPosixPath(filePath);
    const base = path.basename(normalized);
    if (/^[A-Za-z0-9_$]{1,3}\.(?:js|mjs|cjs)$/i.test(base))
        return true;
    if (/-(?:[A-Za-z0-9]{6,})\.(?:js|mjs|cjs)$/i.test(base))
        return true;
    if (/^index-[A-Za-z0-9]{6,}\.(?:js|mjs|cjs)$/i.test(base))
        return true;
    return false;
}
...
function collectObfuscatedSymbolsFromSource(input) {
    ...
    const pushCandidate = (kind, name, node) => {
        if (!name)
            return;
        const obfuscated = kind === "class" ? isLikelyObfuscatedClassName(name) : isLikelyObfuscatedFunctionName(name);
        if (!obfuscated)
            return;
...
function scoreReferenceMatch(input) {
    ...
    for (const token of input.reference.tokens) {
        if (!input.contextKeywords.has(token))
            continue;
        hits.push(token);
    }
...
function buildDeobfuscationTableReport(input) {
    ...
    for (const file of input.jsFiles) {
        ...
        if (!isDeobfuscationCandidateFile(relPath))
            continue;
        if (!fileContextKeywords.has(relPath))
            continue;
        const contextKeywords = fileContextKeywords.get(relPath) ?? new Set();
        if (!isLikelyCoreAppFile(relPath) && contextKeywords.size < 3)
            continue;
        ...
        for (const candidate of symbolCandidates) {
            ...
            if (referencePool.length === 0)
                continue;
            const scopedKeywords = new Set(contextKeywords);
            ...
            if (!bestReference || bestScore < 4.8 || bestHits.length < 2)
                continue;
    ...
    return {
        generatedAtUtc: new Date().toISOString(),
        strategy: "Reference-guided deobfuscation from symbol-map priors and local signal overlap: obfuscated files/classes/functions -> proposed canonical names and reconstructed project paths.",
        referenceInputs: {
...
function buildWebStormTestProject(input) {
    ...
    const reconstructedRoot = (0, exec_1.ensureDir)(path.join(srcRoot, "reconstructed"));
    ...
    let chunkFiles = 0;
    for (const file of input.jsFiles) {
        if (!isLikelyCoreAppFile(file.relPath) && !isDeobfuscationCandidateFile(file.relPath))
            continue;
