...
function patchWebviewCwdNormalization(appDir, options = {}) {
    ...
    const buildNormalizedDisplayPathExpression = (inputExpression) => `(()=>{const __codexWindowsDisplayPathRaw=typeof ${inputExpression}==="string"?${inputExpression}.trim():"";return ${WINDOWS_PATH_CONTRACT.buildInlineNormalizeWindowsPathContractExpression("__codexWindowsDisplayPathRaw", {
...
function resolvePackagedMainEntry(appDir) {
    const pkgPath = path.join(appDir, "package.json");
    const buildDir = path.join(appDir, ".vite", "build");
    const candidates = [];
    if ((0, exec_1.fileExists)(pkgPath)) {
        try {
    ...
    const entryPath = candidates.find((candidate) => (0, exec_1.fileExists)(candidate));
    return entryPath ? { buildDir: path.dirname(entryPath), entryPath } : null;
}
function listBuildJsBundles(buildDir) {
    if (!(0, exec_1.fileExists)(buildDir))
        return [];
    return fs
...
function patchMainPromptPathGuidance(raw) {
    if (raw.includes(MAIN_WINDOWS_PATH_GUIDANCE_PATCH_TAG))
        return raw;
    const replacements = [
        {
