...
exports.shapeImportAliasesIterative = shapeImportAliasesIterative;
...
function toCamelIdentifierFromTokens(tokens) {
...
function isWeakAliasName(name) {
    if (/^[A-Za-z_$]{1,2}$/.test(name))
        return true;
    if (/^[a-z]{2,3}$/.test(name))
        return true;
    if (/^[A-Za-z]{1,2}\d+$/.test(name))
        return true;
    if (/^[a-z][A-Z]$/.test(name))
        return true;
    if (/^[A-Z][a-z]$/.test(name))
        return true;
    if (/^[a-z]{1,3}[A-Z][A-Za-z]?$/.test(name))
        return true;
    return false;
}
function isGenericAliasHintName(name) {
...
function scoreAliasName(name) {
    ...
    if (!/\d/.test(name))
        score += 0.4;
    if (!isWeakAliasName(name))
        score += 1.3;
    if (/^(chunk|dep|helper)/i.test(name))
        score -= 0.2;
    if (/^[a-z]{2,3}$/.test(name))
        score -= 0.8;
    return score;
}
function resolveImportedSourceFile(sourceFile, moduleSpecifier) {
...
function buildFallbackAliasName(importedSymbol) {
...
function shouldRenameIdentifierReference(node) {
...
function collectDeclarationCounts(sourceFile) {
    ...
    const importAliasCountByName = new Map();
    const bump = (map, name) => {
...
function pruneUnusedNamedImports(moduleBody) {
    const sourceFile = ts.createSourceFile("module.ts", moduleBody, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
    const importElementsByLocalName = new Map();
    const elementKey = (element) => `${element.name.getStart(sourceFile)}:${element.name.getEnd()}`;
    ...
    for (const statement of sourceFile.statements) {
        ...
        const replacementText = remainingElements.length > 0
...
function collectAliasUsageHints(sourceFile) {
    const hintsByAlias = new Map();
    const register = (alias, rawHint) => {
        ...
        if (normalizedHint.length < 4)
            return;
        if (isWeakAliasName(normalizedHint))
            return;
        const bucket = hintsByAlias.get(alias) ?? new Map();
...
function shapeImportAliasesPass(input) {
    ...
    const occupiedNames = new Set(Array.from(declarationCountByName.keys()));
    ...
    for (const statement of sourceFile.statements) {
        ...
        for (const element of namedBindings.elements) {
            ...
            if (declarationCount > importAliasCount)
                continue;
            ...
            if (nextAlias === localAlias)
                continue;
            if (!isWeakAliasName(localAlias) && nextScore < localScore + 0.75)
                continue;
            let suffix = 2;
            while (occupiedNames.has(nextAlias) &&
            ...
            if (nextAlias.length === 0 || nextAlias === localAlias || occupiedNames.has(nextAlias))
                continue;
...
function shapeImportAliasesIterative(input) {
    ...
    for (let iteration = 0; iteration < maxIterations; iteration += 1) {
        ...
        if (pass.renamedAliases <= 0 || pass.moduleBody === moduleBody)
            break;
