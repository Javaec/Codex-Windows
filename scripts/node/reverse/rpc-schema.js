...
exports.inferEnvelopeKindsFromText = inferEnvelopeKindsFromText;
exports.buildRpcSchemaReport = buildRpcSchemaReport;
...
function collectPayloadKeysFromObjectLiteral(objectLiteral, objectLiterals, out, helpers, depth = 0) {
    if (depth > 2)
        return;
    for (const property of objectLiteral.properties) {
        if (ts.isSpreadAssignment(property)) {
            ...
            if (nestedSpread)
                collectPayloadKeysFromObjectLiteral(nestedSpread, objectLiterals, out, helpers, depth + 1);
            continue;
        }
...
function extractRpcMethodFromObjectLiteral(input) {
    for (const property of input.objectLiteral.properties) {
        ...
        if (input.helpers.looksLikeRpcMethod(value))
            return value;
    }
    return "";
}
...
function extractRuntimeRpcSignals(input) {
    ...
    for (const line of candidateLines) {
        ...
        for (const method of methods) {
            if (!input.helpers.looksLikeRpcMethod(method))
                continue;
            ...
            if (ts.isCallExpression(node)) {
                ...
                const registerMethod = (method, payloadExpression, payloadObjectLiteral) => {
                    ...
                    if (payloadExpression) {
                        ...
                        if (payloadObject)
                            collectPayloadKeysFromObjectLiteral(payloadObject, objectLiterals, payloadKeys, input.helpers, 0);
                    }
