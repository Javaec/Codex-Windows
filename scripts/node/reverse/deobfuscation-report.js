...
function toProjectRelativeTargetPath(targetProjectPath) {
    ...
    if (compact.startsWith("src-tauri-adapter/"))
        return normalizeModuleExt(compact);
    if (compact.startsWith("tauri/"))
        return normalizeModuleExt(`src-tauri-adapter/${compact.slice("tauri/".length)}`);
    if (compact.startsWith("main/"))
        return normalizeModuleExt(`src/main/${compact.slice("main/".length)}`);
    if (compact.startsWith("renderer/"))
        return normalizeModuleExt(`src/renderer/${compact.slice("renderer/".length)}`);
    if (compact.startsWith("services/"))
        return normalizeModuleExt(`src/services/${compact.slice("services/".length)}`);
    if (compact.startsWith("features/"))
        return normalizeModuleExt(`src/services/features/${compact.slice("features/".length)}`);
    if (compact.startsWith("shared/"))
        return normalizeModuleExt(`src/services/shared/${compact.slice("shared/".length)}`);
    if (compact.startsWith("utils/"))
        return normalizeModuleExt(`src/services/utils/${compact.slice("utils/".length)}`);
    if (compact.length === 0)
        compact = "unknown/module.ts";
