...
function toProjectRelativeTargetPath(targetProjectPath) {
    const normalizeModuleExt = (value) => value.replace(/\.(?:tsx?|jsx|mjs|cjs|js)$/i, ".ts");
    const normalized = toPosixPath(targetProjectPath).replace(/^\.?\//, "");
    const withoutReconstructed = normalized.replace(/^reconstructed\//, "");
    if (withoutReconstructed.startsWith("src-tauri-adapter/"))
        return normalizeModuleExt(withoutReconstructed);
    ...
    if (withoutReconstructed.startsWith("src/main/"))
        return normalizeModuleExt(withoutReconstructed);
    if (withoutReconstructed.startsWith("src/renderer/"))
        return normalizeModuleExt(withoutReconstructed);
    if (withoutReconstructed.startsWith("src/services/"))
        return normalizeModuleExt(withoutReconstructed);
