...
exports.buildComponentBoundariesReport = buildComponentBoundariesReport;
exports.buildDomainReport = buildDomainReport;
exports.formatDomainReportMarkdown = formatDomainReportMarkdown;
...
function countKeywordHits(source, keywords, maxHits, dedupeKeywords) {
...
function hasCoreFile(row, helpers) {
...
function buildComponentBoundariesReport(input) {
    ...
    for (const file of input.jsFiles) {
        ...
        if (!input.helpers.isCandidateBoundaryFile(relPath))
            continue;
        candidateFiles += 1;
        ...
        const referenceRouteHits = countKeywordHits(source, input.referenceProfile.keywordGroups.routes, 10, input.helpers.dedupeKeywords);
        const referenceMethodHits = countKeywordHits(source, input.referenceProfile.keywordGroups.methods, 10, input.helpers.dedupeKeywords);
        const referenceStateHits = countKeywordHits(source, input.referenceProfile.keywordGroups.stateKeys, 10, input.helpers.dedupeKeywords);
        const referenceEventHits = countKeywordHits(source, input.referenceProfile.keywordGroups.events, 10, input.helpers.dedupeKeywords);
        const referenceIpcHits = countKeywordHits(source, input.referenceProfile.keywordGroups.ipc, 10, input.helpers.dedupeKeywords);
        const referenceUiHits = countKeywordHits(source, input.referenceProfile.keywordGroups.ui, 10, input.helpers.dedupeKeywords);
        ...
        if (signalCount > 0)
            filesWithSignals += 1;
        if (!hasComponents && signalCount === 0 && !input.helpers.isLikelyCoreAppFile(relPath))
            continue;
    ...
    for (const [domainKey, domainConfig] of Object.entries(input.domainDefinitions)) {
        ...
        for (const source of sourceRows) {
            for (const row of source.rows) {
                if (row.files.length > 0 && !hasCoreFile(row, input.helpers))
                    continue;
                if (!rowMatchesAnyKeyword(row, domainConfig.keywords))
                    continue;
                ...
                for (const file of row.files) {
                    if (!input.helpers.isLikelyCoreAppFile(file) || input.helpers.isVendorFile(file))
                        continue;
...
function formatDomainReportMarkdown(domainReport, top, domainDefinitions) {
    const sections = [];
