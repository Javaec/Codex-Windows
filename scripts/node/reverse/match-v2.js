...
function normalizeFallbackConfidence(entries) {
    let normalized = 0;
    for (const entry of entries) {
        if (entry.kind !== "class" && entry.kind !== "function")
            continue;
        if (entry.confidence >= 0.65)
            continue;
        if (isGenericReferenceFilePath(entry.reference.file))
            continue;
        const rationaleText = entry.rationale.join(" | ").toLowerCase();
        const fallbackBased = rationaleText.includes("fallback: aggressive-symbol-coverage") ||
        ...
        if (!hasSignals && !hasAnchorSignal)
            continue;
        if (!strongReference && !hasAnchorSignal)
            continue;
