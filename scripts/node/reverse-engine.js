...
exports.runReverseStrictPath = runReverseStrictPath;
exports.runReverseCli = runReverseCli;
...
async function runReverseStrictPath(options) {
    ...
    if (webStormTestProject.checks.build.errors > 0) {
        (0, exec_1.writeWarn)(`Project checks: build errors detected (${webStormTestProject.checks.build.errors}).`);
    }
    if (webStormTestProject.checks.dev.errors > 0) {
        (0, exec_1.writeWarn)(`Project checks: dev runtime errors detected (${webStormTestProject.checks.dev.errors}).`);
    }
...
async function runReverseCli(argv = process.argv.slice(2)) {
    ...
    if (!latestMode) {
        return runReverseStrictPath(options);
    }
    ...
    let runError;
    try {
        resultCode = await runReverseStrictPath(runOptions);
    }
    catch (error) {
        runError = error;
    }
